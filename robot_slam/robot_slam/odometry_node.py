...
class SLAMNode(Node):  
    def __init__(self):
        ...
        self.pose_publisher = self.create_publisher(Odometry, '/pose_datax', 5)
    ...
    def publish_pose(self, pose_data):
        """
        Publish the pose data as a ROS 2 Odometry message.
        ...
        odom_msg.pose.pose.position.x = pose_data["position"]["x"]
        odom_msg.pose.pose.position.y = pose_data["position"]["y"]
        odom_msg.pose.pose.position.z = pose_data["position"]["z"]

        odom_msg.pose.pose.orientation.x = pose_data["orientation"]["x"]
        odom_msg.pose.pose.orientation.y = pose_data["orientation"]["y"]
        odom_msg.pose.pose.orientation.z = pose_data["orientation"]["z"]
        odom_msg.pose.pose.orientation.w = pose_data["orientation"]["w"]

        self.pose_publisher.publish(odom_msg)
        self.get_logger().debug("Odometry data published to '/odom' topic.")
